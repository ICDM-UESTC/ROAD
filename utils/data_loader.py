...
class ConvECorpus(Corpus):
    ...
    def get_validation_pred(self, model, split='test'):
        ...
        for batch_num in range(max_batch_num):
            ...
            pred, attention ,conf_outputs = model.forward(head_batch_indices)
            ...
            conf_s_pred, conf_i_pred, conf_t_pred, conf_m_pred = conf_outputs
            conf_stack = torch.cat([conf_s_pred, conf_i_pred, conf_t_pred, conf_m_pred], dim=1)
            conf_weights = conf_stack / (conf_stack.sum(dim=1, keepdim=True) + 1e-8)
            pred[0] = conf_weights[:, 0].unsqueeze(1) * pred[0]
            pred[1] = conf_weights[:, 1].unsqueeze(1) * pred[1]  
            pred[2] = conf_weights[:, 2].unsqueeze(1) * pred[2] 
            pred[3] = conf_weights[:, 3].unsqueeze(1) * pred[3]
            ...
            pred, attention,conf_outputs= model.forward(tail_batch_indices)
            for i in range(pred[0].shape[0]):
